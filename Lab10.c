...
uint8_t collide = 0; //main loops executes while this is 0 - if set to 1 game ends
uint8_t speed = 1; //speed initially 20 pixels/second, increases/decreases depending on button input
uint8_t NeedToDraw;//1 need to draw 0 not
uint32_t TimeScore = 0; //used later
void Init(void){ //Initializes the spirites - col structure is used for explosion animation later
	...
	PCar.image = Maincar;
	PCar.life = alive;
  for(uint8_t i=0;i<1;i++){ //first three cars 30 vertical units apart
		Enemys[i].x = 10;
		Enemys[i].y = 30*i+10;
    Enemys[i].image = Squirrelc;
    Enemys[i].life = alive;
  }
	for(uint8_t i=1;i<2;i++){ //first three cars 30 vertical units apart
		Enemys[i].x = 10;
		Enemys[i].y = 30*i;
    Enemys[i].image = frogc;
    Enemys[i].life = alive;
  }
	for(uint8_t i=2;i<3;i++){ //first three cars 30 vertical units apart
		Enemys[i].x = 10;
		Enemys[i].y = 30*i;
    Enemys[i].image = earc;
    Enemys[i].life = alive;
...
speeddown = Switch_In()&0x02; //PE1
 ...
 void Collisions(void){int x1, y1, y2, x2;
	...
	for(int i = 0; i<3; i++){
		x1 = Enemys[i].x + 6;
		y1 = Enemys[i].y + 4;
		if((x1 >> 4) == (x2 >> 4) && (y1 >> 3) == (y2 >> 3)){ //short way of collision detection
			col.x = PCar.x;
			col.y = PCar.y;
			col.life = alive;
			collide = 1;
...
void DDraw(void){int i;
	...
	SSD1306_DrawUDec2digit(63, 10, TimeScore, SSD1306_INVERSE);
...
void Score_Init(void){
	Timer1_Init(&Keep_Score, 80000000); //1 Hz
}
...
int main(void){
 ...
 // TExaS_Init(&LogicAnalyzerTask); // 2) call to activate logic analyzer
  ...
  SSD1306_ClearBuffer();
  SSD1306_DrawBMP(2, 62, speedracer, 0, SSD1306_WHITE);
  SSD1306_OutBuffer();
  Delay100ms(20);
  ...
  SSD1306_OutString("Nice try Racer!");
  SSD1306_SetCursor(1, 3);
  SSD1306_OutString("You are a master");
	SSD1306_SetCursor(1, 4);
	SSD1306_OutString("Congratulations!");
  }
	
	if(lang == 2 && TimeScore < 100){
	SSD1306_OutClear();  
  SSD1306_SetCursor(1, 1);
  ...
  SSD1306_OutString("Buen intento");
  SSD1306_SetCursor(1, 3);
  SSD1306_OutString("Eres un maestro");
	SSD1306_SetCursor(1, 4);
